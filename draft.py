    ...
    for t in range(T):
        ...
        if in_care:
            for patient in in_care.copy():
                if patient.actual_severity_score_1 == 0 or (patient.actual_severity_score_1 == 1 and patient.actual_severity_score_2 < 0.5):
...
def main():
    ...
    healing_df = pd.DataFrame(healing_log, columns=["Patient ID", "Start Care Time", "Departure Time",
                                                    'init severity', "Severity Score 1", "Severity Score 2", 'actual_1', 'actual_2'])
