    ...
    for t in range(T):
        ...
        if in_care:
            for patient in in_care.copy():
                if  patient.actual_severity_score_1== 0 or (patient.actual_severity_score_2 < 0.5 and patient.actual_severity_score_1 == 1):
        ...
        if waiting_room and len(in_care) < M:  # Check if there's space in care
            ...
            waiting_room.sort(key=lambda x: (-x.severity_score_1, -x.severity_score_2, x.subject_id))  # Highest first
        ...
        while len(in_care) == M and waiting_room:  # Only check for switches if care is full
            ...
            lowest_severity_patient = min(in_care, key=lambda x: (x.severity_score_1, x.severity_score_2))
            highest_severity_patient = max(waiting_room, key=lambda x: (x.severity_score_1, x.severity_score_2))
...
def main():
    ...
    arrival_df.to_csv('sim2/arrival_data.csv', index=False)
    healing_df.to_csv('sim2/healing_data.csv', index=False)
    left_df.to_csv('sim2/left_without_care_data.csv', index=False)
